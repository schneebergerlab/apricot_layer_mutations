...
# 
# 
# file_paths = list.files(path = '/srv/netscratch/dep_mercier/grp_schneeberger/projects/SynSearch/tests/rtiger_input/',pattern = 'input_corrected', full.names = TRUE)
# sampleIDs <- basename(file_paths)
# # Create the expDesign object
# expDesign = data.frame(files=file_paths, name=sampleIDs)
# chr_len <- RTIGER::ATseqlengths
# # names(chr_len) <- c('Chr1' , 'CURChr2', 'CURChr3', 'CURChr4', 'CURChr5')
# names(chr_len) <- c('Chr1' , 'Chr2', 'Chr3', 'Chr4', 'Chr5')
# library(Gviz)
#  
# 
# myDat <- RTIGER(expDesign = expDesign,
#                 rigidity = 500,
