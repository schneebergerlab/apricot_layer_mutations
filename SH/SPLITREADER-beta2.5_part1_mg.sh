...
TESequences=/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/scdna/bigdata/variant_calling/pheno_mut/TE_indel/splitreader/TE_library.fa
...
if [ -z "$pe" ]
then
  pe=`samtools view -c -f 1 $InputDir/${in}${ext}.bam | awk '{print $1}'`
else
  if [[ "$pe" == F* ]]
  then
    pe=""
...
samtools view -@ $CORES -h -f 4 $InputDir/${in}${ext}.bam > $TmpDir/${in}${ext}.sam 2>> $TmpDir/log.txt
samtools view -@ $CORES -f 8 $InputDir/${in}${ext}.bam >> $TmpDir/${in}${ext}.sam 2>> $TmpDir/log.txt
# convert to BAM
samtools sort -l 0 -n -@ $CORES -O BAM $TmpDir/${in}${ext}.sam > $TmpDir/${in}${ext}.bam
...
#  java -Xmx10g -XX:+UseSerialGC -Djava.io.tmpdir=$TmpDir/javatemp -jar $picardDir SamToFastq -INPUT $TmpDir/${in}${ext}.bam -FASTQ $readsDir/$in.1 -SECOND_END_FASTQ $readsDir/$in.2 -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
  samtools fastq -@ 20 -1 $readsDir/$in.1 -2 $readsDir/$in.2 -s /dev/null -0 /dev/null -n $TmpDir/${in}${ext}.bam
  if [ -z "$LS" ]
    then
    LS=300
...
#  samtools view -@ $CORES -hF 4 $InputDir/${in}${ext}.bam | awk -v l=$LS '(($4-$8)>(l*10) || ($8-$4)>(l*10) || $7!="=") || $1~/@HD/ || $1~/@SQ/ || $1~/@PG/' | java -jar -Xmx10g -Djava.io.tmpdir=$TmpDir/javatemp $picardDir SamToFastq -INPUT /dev/stdin -FASTQ $readsDir/$in.1b -SECOND_END_FASTQ $readsDir/$in.2b -TMP_DIR $TmpDir/javatemp  2>> $TmpDir/log.txt
  samtools view -@ $CORES -hF 4 $InputDir/${in}${ext}.bam \
  | samtools sort -n -@ $CORES -O SAM - \
  | awk -v l=$LS '(($4-$8)>(l*10) || ($8-$4)>(l*10) || $7!="=") || $1~/@HD/ || $1~/@SQ/ || $1~/@PG/' \
  | samtools fastq -@ 20 -1 $readsDir/$in.1b -2 $readsDir/$in.2b -s /dev/null -0 /dev/null -
...
samtools view -@ $CORES -H $TmpDir/$in.sam > $TmpDir/$in-TE.sam 2>> $TmpDir/log.txt
samtools view -@ $CORES -F 4 -S $TmpDir/$in.sam | awk '$6~/^[2-8][0-9]S/ || $6~/[2-8][0-9]S$/ || $6~/^1[0-9][0-9]S$/ || $6~/1[0-9][0-9]S$/ {print $0}'>> $TmpDir/$in-TE.sam 2>> $TmpDir/log.txt # -f 1
  
  ### extract unmapped pair of mapped reads
samtools view -@ $CORES -f 4 -F 8 -S $TmpDir/$in.sam | awk '{print $0}' >> $TmpDir/$in-TE.sam 2>> $TmpDir/log.txt # -f 5
  ### extract mapped pair ends of unmapped reads
samtools view -@ $CORES -f 8 -F 4 -S $TmpDir/$in.sam | awk '{print $0}' >> $TmpDir/$in-TE.sam 2>> $TmpDir/log.txt # -f 5

### convert sam to bam
# cp $TmpDir/$in-TE.sam $OutputDir/$in-TE.sam
samtools view -@ $CORES -Sb $TmpDir/$in-TE.sam > $OutputDir/$in-TE.bam
