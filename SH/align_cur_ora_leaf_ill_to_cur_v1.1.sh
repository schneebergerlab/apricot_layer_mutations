...
for sample in ${samples[@]}; do
  cd ${cwd}/$sample
  bsub -q multicore20 -n 20  -R "span[hosts=1] rusage[mem=50000]" -M 60000 -oo align_${sample}.log -eo align_${sample}.err "
    minimap2 -ax sr -t 20 \
      --secondary=no \
      $curidx \
    ...
    samtools index -@10 ${sample}.DUPmarked.bam
    ...
    samtools view -@20 -q10 -b ${sample}.dedup.sorted.bam > ${sample}.q10.dedup.sorted.bam
    samtools index -@20 ${sample}.q10.dedup.sorted.bam

    cat $cur_contig_list \
    | sed 's/\\n/ /g' \
    | xargs -d '\n' -n 1 -P 20 -I {} /bin/bash -c ' \
        bcftools mpileup -q \$2 -d 800 --threads 1 -f \$3 -r \$1 \${4}.dedup.sorted.bam  > \${4}_mq\${2}_\${1}.pileup \
      ' -- {} $mq $refcur ${sample}.q10

    bcftools concat --threads 20 ${sample}.q10_mq${mq}_*pileup \
    | bcftools call --threads 20 -m -v -Ov --ploidy-file ${ploidy} \
