...
def main(argv):
    ...
    ref = os.path.basename(refProtFas).split(".")[0]
    alt = os.path.basename(altProtFas).split(".")[0]
    print(ref,"ref")
    ...
    [misMerBlp,misMerBlpRef,misMerBlpAlt]= findMisMerBlp(blastpFlt1,outdir,minId,maxSplitCov,maxIdf,refIdx,altIdx,altLen,altPos, refLen)

    #check mis-splitting genes
    [misSplBlp,misSplBlpRef,misSplBlpAlt]= findMisSplBlp(blastpFlt1,outdir,minId,maxSplitCov,maxIdf,refIdx,altIdx,refLen,altPos)
    ...
    [misMerBln, misMerBlnRef, misMerBlnAlt] = findMerBlastn(inFile, ungrRef, ungrAlt)  #misMerBln[altId]=[[refID1,ugr/gr],[refID2,ungr/gr],.]

    inFile = outdir + "/alt-genome.mis-split.ref-gene.txt"
    [misSplBln, misSplBlnRef, misSplBlnAlt] = findSplitBlastn(inFile, ungrRef, ungrAlt)

    inFile = outdir + "/alt-genome.mis-exon.ref-gene.txt"
    [misExon, misExRef,misExAlt, misUngrRef,misUngrAlt]  = findExBlastn(inFile, ungrRef, ungrAlt)

    accBlastnRef = getAccBlastnRef(altBlnRefRes)
    [grNumRef, grNumAlt, refN, altN, refCNV, altCNV] = getGroupNum(groupFile,ref)
    ...
    refUnknown={}
    ss = ["misMerBlpBln", "misSplBlpBln", "misMerBlp", "misMerBln", "misSplBlp", "misSplBln", "misExon","misUnGr", 
          "unAnn", "unAss", "paAss9090", "paAss9060", "paAss9030","cnv-NN","cnv-NM","unknown"]
    refGeneStat = {
     "misMerBlpBln":[0,0,0], "misSplBlpBln":[0,0,0], "misMerBlp":[0,0,0], "misMerBln":[0,0,0], "misSplBlp":[0,0,0], "misSplBln":[0,0,0], 
     "misExon":[0,0,0], "misUnGr":[0,0,0],
     "unAnn":[0,0,0], "unAss":[0,0,0], "paAss9090":[0,0,0], "paAss9060":[0,0,0], "paAss9030":[0,0,0],
     "cnv-NN":[0,0,0],"cnv-NM":[0,0,0],"unknown":[0,0,0]
     }
    for gene in sorted(refGeneBed.keys()) :
        flag = "xxx"
        if gene in misMerBlpRef.keys() and gene in misMerBlnRef.keys():
            flag = "misMer-BlpBln"
            refGeneStat["misMerBlpBln"][0] += 1
            #refGeneStat["misMerBln"][0] += 1
            if gene in ungrRef.keys() : 
            	refGeneStat["misMerBlpBln"][1] += 1
            	#refGeneStat["misMerBln"][1] += 1
            elif gene in grNumRef.keys():
            	refGeneStat["misMerBlpBln"][2] += 1
            	#refGeneStat["misMerBln"][2] += 1
        elif gene in misMerBlpRef.keys ()  :
            flag = "misMer-Blp"
            refGeneStat["misMerBlp"][0] += 1
            if gene in ungrRef.keys():
            	refGeneStat["misMerBlp"][1] += 1
            elif gene in grNumRef.keys() :
            	refGeneStat["misMerBlp"][2] += 1
            ...
            if gene in ungrRef.keys():
            	...
            	refGeneStat["misMerBln"][2] += 1
        elif gene in misSplBlpRef.keys() and gene in misSplBlnRef.keys ():
            flag = "misSpl-BlpBln"
            refGeneStat["misSplBlpBln"][0] += 1
            
            if gene in ungrRef.keys():
                refGeneStat["misSplBlpBln"][1] += 1
                #refGeneStat["misSplBln"][1] += 1
            elif gene in grNumRef.keys():
                refGeneStat["misSplBlpBln"][2] += 1
                #refGeneStat["misSplBln"][2] += 1	
        elif gene in misSplBlpRef.keys() :
            flag = "misSpl-Blp"
            refGeneStat["misSplBlp"][0] += 1
            if gene in ungrRef.keys():
                refGeneStat["misSplBlp"][1] += 1
            elif gene in grNumRef.keys():
            	refGeneStat["misSplBlp"][2] += 1
            	
        elif gene in misSplBlnRef.keys():
            flag = "misSpl-Bln"
            refGeneStat["misSplBln"][0] += 1
            if gene in ungrRef.keys():
                refGeneStat["misSplBln"][1] += 1
            elif gene in grNumRef.keys() :
            	refGeneStat["misSplBln"][2] += 1
            ...
            flag = "misExon"
            refGeneStat["misExon"][0] += 1
            if gene in ungrRef.keys():
                refGeneStat["misExon"][1] += 1
            elif gene in grNumRef.keys() :
            	refGeneStat["misExon"][2] += 1
        elif gene in misUngrRef.keys():
            flag = "misUnGr"
            refGeneStat["misUnGr"][0] += 1
            if gene in ungrRef.keys():
                refGeneStat["misUnGr"][1] += 1
            elif gene in grNumRef.keys() :
            	refGeneStat["misUnGr"][2] += 1
        elif gene in unAssRef.keys() : ## deleted and partially deleted
            flag = unAssRef[gene]
            refGeneStat[flag][0] += 1
            if gene in ungrRef.keys():
                refGeneStat[flag][1] += 1
            elif gene in grNumRef.keys() :
            	refGeneStat[flag][2] += 1
        elif gene in unAnnRef.keys(): # unannotated
            flag = "unAnn"
            refGeneStat[flag][0] += 1
            if gene in ungrRef.keys():
                refGeneStat[flag][1] += 1
            elif gene in unAnnRef.keys() :
            	refGeneStat[flag][2] += 1
            ...
            n2 = int(flag.split("-")[1])
            if n1 == n2 :
                refGeneStat["cnv-NN"][0] += 1
                if gene in ungrRef.keys():
                    refGeneStat["cnv-NN"][1] += 1
                elif gene in grNumRef.keys() :
                	refGeneStat["cnv-NN"][2] += 1
            else :
                refGeneStat["cnv-NM"][0] += 1
                if gene in ungrRef.keys():
                    refGeneStat["cnv-NM"][1] += 1
                elif gene in grNumRef.keys() :
                	refGeneStat["cnv-NM"][2] += 1
        else:
            flag = "unknown"
            refUnknown[gene] = 1
            refGeneStat["unknown"][0] +=1
            if gene in ungrRef.keys():
                refGeneStat["unknown"][1] +=1
            elif gene in grNumRef.keys() :
            	refGeneStat["unknown"][2] += 1
                
        bestp = "xx\txx\txx\txx"
        if bestBlastp.has_key(gene) :
            bestp = "\t".join(str(x) for x in bestBlastp[gene])
        fo.write("{}\t{}\t{}\t{}\t{}\n".format(refGeneBed[gene], gene, flag, ";".join(refBest[gene]), bestp))
    fo.close()

    for k in ss :
        fos.write("{}\t{}\n".format(k, "\t".join(str(x) for x in refGeneStat[k]) ))
    fos.write("{}\t{}\t{}\t{}\n".format("total", refN, len(ungrRef) , refCNV ))
    fos.close()
    ...
    for gene in sorted(altGeneBed.keys()) :
        flag = "xxx"
        if gene in misMerBlpAlt.keys() and gene in misMerBlnAlt.keys():
            flag = "misMer-BlpBln"
            altGeneStat["misMerBlpBln"][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat["misMerBlpBln"][1] += 1
               
            elif gene in grNumAlt.keys() :
            	altGeneStat["misMerBlpBln"][2] += 1
                #altGeneStat["misMerBln"][2] += 1
        elif gene in misMerBlpAlt.keys ()  :
            flag = "misMer-Blp"
            altGeneStat["misMerBlp"][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat["misMerBlp"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misMerBlp"][2] += 1
        elif gene in misMerBlnAlt.keys ():
            flag = "misMer-Bln"
            altGeneStat["misMerBln"][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat["misMerBln"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misMerBln"][2] += 1
            	
        elif gene in misSplBlpAlt.keys() and gene in misSplBlnAlt.keys ():
            flag = "misSpl-BlpBln"
            altGeneStat["misSplBlpBln"][0] += 1
            
            if gene in ungrAlt.keys():
                altGeneStat["misSplBlpBln"][1] += 1
                #altGeneStat["misSplBln"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misSplBlpBln"][2] += 1
                #altGeneStat["misSplBln"][2] += 1
        elif gene in misSplBlpAlt.keys() :
            flag = "misSpl-Blp"
            altGeneStat["misSplBlp"][0] += 1
            if gene in ungrAlt.keys() :
                altGeneStat["misSplBlp"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misSplBlp"][2] += 1
        elif gene in misSplBlnAlt.keys():
            flag = "misSpl-Bln"
            altGeneStat["misSplBln"][0] += 1
            if gene in ungrAlt.keys() :
                altGeneStat["misSplBln"][1] += 1
            elif gene in grNumAlt.keys():
            	altGeneStat["misSplBln"][2] += 1
            ...
            flag = "misExon"
            altGeneStat["misExon"][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat["misExon"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misExon"][2] += 1
        elif gene in misUngrAlt.keys():
            flag = "misUnGr"
            altGeneStat["misUnGr"][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat["misUnGr"][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat["misUnGr"][2] += 1
        elif gene in unAssAlt.keys() :
            flag = unAssAlt[gene]
            altGeneStat[flag][0] += 1
            if gene in ungrAlt.keys():
                altGeneStat[flag][1] += 1
            elif gene in grNumAlt.keys() :
            	altGeneStat[flag][2] += 1
        elif gene in unAnnAlt.keys():
            flag = "unAnn"
            altGeneStat["unAnn"][0] += 1
            if gene in ungrAlt.keys():
			    altGeneStat[flag][1] += 1
            elif gene in grNumAlt.keys() :
				altGeneStat[flag][2] += 1
            ...
            n2 = int(flag.split("-")[1])
            if n1 == n2 :
                altGeneStat["cnv-NN"][0] += 1
                if gene in ungrAlt.keys():
				    altGeneStat["cnv-NN"][1] += 1
                elif gene in grNumAlt.keys() :
					altGeneStat["cnv-NN"][2] += 1
            else :
                altGeneStat["cnv-NM"][0] += 1
                if gene in ungrAlt.keys():
				    altGeneStat["cnv-NM"][1] += 1
                elif gene in grNumAlt.keys() :
					altGeneStat["cnv-NM"][2] += 1
        else:
            flag = "unknown"
            refUnknown[gene]=1
            altGeneStat[flag][0] += 1
        ...
        fo.write("{}\t{}\t{}\t{}\t{}\n".format(altGeneBed[gene], gene, flag, ";".join(altBest[gene]), bestp))
    fo.close()

    for k in ss :
        fos.write("{}\t{}\n".format(k, "\t".join(str(x) for x in altGeneStat[k]) ))
    fos.write("{}\t{}\t{}\t{}\n".format("total", altN, len(ungrAlt), altCNV ))
    fos.close()

    '''step 3: combine the above information to update alt-gene or adde genes to alt-genome '''
    fi = open(altBed,"r")
    ...
    while True :
        ...
        if ungrAlt.has_key(id) :
            ...
            gr = grNumAlt[id]
        if misMerBln.has_key(id) :
            refGene = misMerBln[id][0]
            grRef = misMerBln[id][1]
            for k in range(len(misMerBln[id])) :
                if k == 0 : continue
                if k % 2 == 0 :
                    refGene = refGene + "," + misMerBln[id][k]
                    grRef = grRef + "," + misMerBln[id][k+1]
            flag = "mis-merge" + "\t" + refGene + "\t" + grRef
            
        elif misMerBlp.has_key(id) :
            refGene = ",".join(misMerBlp[id])
            gr = []
            for g in misMerBlp[id]:
            	if ungrRef.has_key(g) :
            		gr.append("ungrouped")
            	else:
            		gr.append("grouped")
            grRef = ",".join(gr)
        ...
        elif misSplBln.has_key(id) :
            refGene = misSplBln[id][0]
            flag = "mis-split" + "\t" + refGene + "\t" + misSplBln[id][1]

        elif misSplBlp.has_key(id):
            refGene = misSplBlp[id][0]
            flag = "mis-split" + "\t" + refGene + "\t" + misSplBlp[id][1]
...
def getBesthit(inFile):
    ...
    while True :
        ...
        if not line : break
        t = line.strip().split("\t")
        besthit[t[3]] = [t[0], t[1], t[2], t[4], t[5]]
    fi.close()
    return besthit
...
def getGroupNum (groupFile,ref):
    ...
    grNumRef, grNumAlt = {}, {}
    #ref = "AT"
    [refN, altN, refCNV, altCNV] = [0,0,0,0] 
    while True :
        line = fi.readline()
        if not line : break
        t = line.strip().split()
        ...
        for k in range(len(t)) :
            ...
            if re.search(ref,t[k]) :
                ...
                altIds.append(id)
        for k in altIds :
            grNumAlt[k] = str(len(refIds)) + "-" + str(len(altIds))
        for k in refIds :
        	grNumRef[k] = str(len(refIds)) + "-" + str(len(altIds))
        refN += len(refIds)
        altN += len(altIds)
        if len(refIds) != len(altIds) :
        	refCNV += len(refIds)
        	altCNV += len(altIds)
    fi.close()
    return grNumRef, grNumAlt, refN, altN, refCNV, altCNV
...
def findMerBlastn(inFile, ungrRef, ungrAlt):
    ...
    while True :
        ...
        if ungrRef.has_key(refId) :
            gr = "ungrouped"
            k1 += 1
        misMerBlnRef[refId] = gr
        misMerBlnAlt[altId] = gr
        if ungrAlt.has_key(altId) :
            k2 += 1

        if not misMerBln.has_key(altId) :
            misMerBln[altId]=[refId,gr]
        else :
            misMerBln[altId].append(refId)
            misMerBln[altId].append(gr)
    fi.close()
    print "## find mis-merged gene by blastn"
    print "mis-merged genes alt number: ", len(misMerBln)
    print "mis-merged genes ref number: ", len(misMerBlnRef)
    print "mis-merged genes resulted in ungroupd Ref ", k1
    print "mis-merged genes resulted in ungroupd Alt ", k2
    return misMerBln, misMerBlnRef, misMerBlnAlt
...
def findSplitBlastn(inFile, ungrRef, ungrAlt):
    ...
    return [misSplit,misSplitRef,misSplitAlt]
...
def findExBlastn(inFile, ungrRef, ungrAlt):
    ...
    print "\n # alt-gene have a besthit in a gene region of ref-genome, but they are not ortholog"
    print " alt-genes number", len(misExon)
    print "		resulted in ungrouped ref-gene", k1
    print "		resulted in ungrouped alt-gene", k2

    print " mis-ungrouped: ref-gene and ungrouped", len(misUngrRef), k31
    print " mis-ungrouped: acc-gene and ungrouped", len(misUngrAlt), k32
    
    return [misExon, misExRef, misExAlt, misUngrRef, misUngrAlt]
...
def findRefMM(blastnRes,refBed,altBed,outdir, ungrAlt, ungrRef, rnaGff, blastp):
    ...
    while True :
        ...
        if t[0] == "None" :
            ...
            unAssAlt[id] = "paAss9090"
            k9090 += 1
            if float(t[4]) < 60 :
            	k9060 += 1
            	unAssAlt[id] = "paAss9060"
            if float(t[4]) < 30:
            	k9030 += 1
            	unAssAlt[id] = "paAss9030"
    ...
    print("# findRefMM: unassembled or partially assembled alt gene \n #del100\tI90C90\tI9060\tI9030")
    print("  {}\t{}\t{}\t{}".format(k100,k9090,k9060,k9030))
    ...
    while True :
        ...
        if float(t[5]) <= 85 or t[6] != ".": continue
    ...
    fo.close()
    print "#  Gene annotated in alt-genome but not in ref, blastn besthit (iden>85) intersect with ref-genome :" , len(unAnnAlt)
    print "		unAnnAltSt1 " , unAnnAltSt1 
    print "		unAnnAltSt2 " , unAnnAltSt2
    ...
    fi = open(outdir + "/ref-genome.missing.alt-gene.txt", "r" )
    ...
    fo.close()
    print "   alt-genes are not annotated in ref-genome but supported with RNA-seq transcript", len(rnaSup)
    ...
    while True :
        ...
        if not ungrAlt.has_key(altId) :
            continue
        if  float(t[4]) < 80 or float(t[5]) < 85 or float(t[-1])/(float(t[2]) - float(t[1])) < 0.8 or  float(t[-1])/(float(t[8]) - float(t[7])) < 0.8:
            continue
        ...
        if not ungrRef.has_key(refId) :
            k2 += 1
        if blastp.has_key(t[3] + refId) and  float(blastp[t[3] + refId][0]) > 85 :
            k3+=1
            continue
    ...
    fo.close()
    print "ungroupd alt-genes have blast hit region (iden>85, cov > 80) which intersects with a gene in ref-genome (both cov>80) but they are not orthologs: " , k1
    print "	 but the Ref has ortholog: " , k2
    print "  but they have high similarity (>85): " , k3
...
def findMisMerBlp (blastpFlt,outdir,minId,maxSplitCov,maxIdf,refIdx,altIdx,altLen,altPos, refLen):
    ...
    os.system("sort -k1,1 -k2,2 -k7,7n " + blastpFlt + "  > " + outdir + "/blastp.flt.srt1.out") ## sorted by ref gene-idx
    ...
    alt = "G"
    preRefCov = 0
    print "\n## Method 1: find mis-merged genes in accession based on the blastp results "
    while True :
        line = fi.readline()
        if not line : break
        t = line.strip().split("\t")
        if re.search(alt,t[0]) and re.search(ref,t[1]) :
            ...
            if (aIdx == altIdx[aId] and refIdx[rId] == rIdx + 1 and abs(float(t[2])- prevIden) < maxIdf and preCov < maxSplitCov
                    and float(t[3])/altLen[aId] < maxSplitCov  and float(t[-1]) > maxSplitCov and preRefCov > maxSplitCov) :
                [chrom,start,end]=altPos[aId]
                
                if aId not in misMerGene.keys():
                    misMerGene[aId]  = []
                    if prevRefGene not in misMerGene[aId]:
                        misMerGene[aId].append(prevRefGene)
                    if rId not in misMerGene[aId]:
    ...
    fo.close()
    print "ref-genes are mismerged in alt-genome (alt-genes): ",len(misMerGene)
    print "ref-genes are mismerged in alt-genome (ref-genes): : ",len(misMerGeneRef)
...
def findMisSplBlp (blastpFlt,outdir,minId,maxSplitCov,maxIdf,refIdx,altIdx,refLen,altPos):
    print "\n## Method 1: find mis-split genes in accession based on the blastp results "
    os.system("sort  -k2,2 -k1,1 -k9,9n " + blastpFlt + "  > " + outdir + "/blastp.flt.srt2.out") ## sorted by alt gene-index
    ...
    fo.close()
    print "ref-genes are mis-split  in alt-genome (alt-genes): ",len(altMisSplitAlt)
    print "ref-genes are mis-split  in alt-genome (ref-genes): : ",len(altMisSplitRef)
...
def findAltMM (refBlnAltRes,altBed,outdir,ort,ungrRef,ungrAlt,refIdx,altIdx, blastp):
    ...
    print "\n## Method 2: (findAltMM) find Ref genes which are not assembled (could be deleted) in other accession based on result of Ref-gene blastn against alt genome "
    ...
    os.system("intersectBed -a " + blastnRes2 + " -b " + altBed + " -wo | \
    awk '{if ($6>90) print}' |cut -f 10 |sort -k1,1 |uniq -d -c |sed 's/ \+/\t/g' > " + outdir + "/alt-gene.multiple.blastn-hit.txt")
    
    os.system("intersectBed -a " + blastnRes2 + " -b " + altBed + " -wo | \
    awk '{if ($6>90) print}' |cut -f 4 |sort -k1,1 |uniq -d -c  |sed 's/ \+/\t/g' > " + outdir + "/ref-gene.multiple.blastn-hit.txt")
    ...
    print "\n### Genes annotated in ref-genome but not in alt, blastn besthit (iden>85) intersect with alt-genome"
    print " Both [cov0, cov30, cov60, cov90]  ", unAnnRefSt1
    print " ungr [cov0, cov30, cov60, cov90]  ", unAnnRefSt2
    print " k1 (intersectBed no repeat hit [no mis-split or mis-merge])", k1 


    for k in sorted(repHitM1.keys()) :
        if len(repHitM1[k]) == 2 :
            altId1 = repHitM1[k][0][9]
            #altId1 = altId1.split("=")[1]
            altId2 = repHitM1[k][1][9]
            #altId2 = altId2.split("=")[1]
            if ( ( float(repHitM1[k][0][8]) - float(repHitM1[k][0][7]) ) * 0.6 < float(repHitM1[k][0][-1]) \
                and ( float(repHitM1[k][1][8]) - float(repHitM1[k][1][7]) ) * 0.9 < float(repHitM1[k][1][-1]) and 
                abs(altIdx[altId1] - altIdx[altId2]) == 1 ) :
                t1 = repHitM1[k][0]
                t2 = repHitM1[k][1]
                ...
                fo3.write("\n")
            elif ( ( float(repHitM1[k][0][8]) - float(repHitM1[k][0][7]) ) * 0.9 < float(repHitM1[k][0][-1]) \
                and ( float(repHitM1[k][1][8]) - float(repHitM1[k][1][7]) ) * 0.6 < float(repHitM1[k][1][-1]) and 
                abs(altIdx[altId1] - altIdx[altId2]) == 1 ) :
                t1 = repHitM1[k][0]
                t2 = repHitM1[k][1]
                #fo3.write("\t".join(t1[0:11]))
    ...
    for k in sorted(repHitM2.keys()) :
        if len(repHitM2[k]) == 2 :
            ...
            for i in range(len(repHitM2[k])) :
                ...
                #if k == "ORA1G.1069" : 
                	#print refId, repHitM2[k][i]    
                    #print          
                if (float(repHitM2[k][i][2]) - float(repHitM2[k][i][1]) ) * 0.9 < float(repHitM2[k][i][-1]):
    ...
    return [unAssRef,unAnnRef,misExRef,misExAlt]
