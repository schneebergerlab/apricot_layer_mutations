...
#sys.path.insert(0, '/srv/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/annotations/scripts/util/')
#from evm.pasa.integrate.pipeline import augMerge1, cmdExonerate
...
#    RUNLOG = os.path.join( runCfg["workDir"], "TMP.log" )
    LOG = open(RUNLOG,"w", 1)
    ...
    tmp0 = glob.glob(runCfg["rnaseqDataDir"] + "/*.fastq*")
    rnaseqFastqs0 = [x for x in tmp0 if not re.search(r"_1.fastq",x) and not re.search(r"_2.fastq", x)]
    ...
    for i in range(len(rnaseqFastqs)) :
        ...
        elif re.search(r'.fastq.gz', name) :
    ...
    cmd = "stringtie --merge -p 1 -o " + rnagtf + " " + rnalist
    print("run ... ", cmd, "\n")
    os.system(cmd)
    LOG.write(cmd + "\n")
    ...
    evmOutFiles2 = list(filter(lambda x: 'chr' not in x , evmOutFiles2))
    evmOutFiles2.sort()
    ...
    evmGffFiles2 = list(filter(lambda x: 'chr' not in x , evmGffFiles2))
    evmGffFiles2.sort()
