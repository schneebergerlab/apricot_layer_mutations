...
sdict['MUT_11_1']=83
sdict['MUT_15']=80
# Cluster reads to barcodes and deduplicate them (https://isoseq.how/umi/)
...
# <editor-fold desc="Get read count for each BC">
...
# Get stat plots
python.analysis_plots.expression_plots

# Separate Iso-seq reads to individual clusters
python.iso_seq_analysis.get_allele_freq_at_sm_pos_plot()

# Map iso-seq reads from individual clusters to cur reference and get read-count at SM positions
refcur='/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/data/assemblies/hifi_assemblies/cur.genome.v1.fasta'
cwd='/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/isoseq/get_cells/'
cd $cwd
paftools.js gff2bed /netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/data/assemblies/hifi_assemblies/cur.pasa_out.3utr.gene_rrna.sort.gtf > cur.pasa_out.3utr.gene_rrna.sort.bed
for s in WT_1 WT_19 MUT_11_1 MUT_15; do
    cd $cwd; cd $s
    ...
    for c in ${clst[@]}; do
        ...
        bsub -q multicore20 -n 10 -R "span[hosts=1] rusage[mem=20000]" -M 25000 -oo ${cls}.log -eo ${cls}.err "
            minimap2 -ax splice:hq -t 10 -R '@RG\tID:${cls}\tSM:1' --secondary=no --junc-bed ../cur.pasa_out.3utr.gene_rrna.sort.bed $refcur $c \
...
for s in WT_1 WT_19 MUT_11_1 MUT_15; do
    cd $cwd; cd $s
#    samtools depth -d 0 -o ${s}.iso_seq.depth.txt --reference $refcur ${s}.iso_seq.bam
    cut -f 3 ${s}.iso_seq.depth.txt | sort -n | uniq -c > ${s}.iso_seq.depth.hist &
done
...
for s in WT_1 WT_19 MUT_11_1 MUT_15; do
    ...
    while IFS=$'\t' read -r -a r; do
        samtools mpileup -A -d 100000 -q 0 -Q 0 -f $refcur --output-QNAME -r ${r[0]}:${r[2]}-${r[2]} ${s}.iso_seq.bam >> all_sm_snp_expressed.pileup
    done < $smfin
done
# </editor-fold>


# <editor-fold desc="Full length transcript analysis. Trying to find allele-specifc expression">
# https://isoseq.how/classification/pigeon.html
