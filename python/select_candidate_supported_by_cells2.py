...
if __name__ == '__main__':
    ...
    # D1 = '/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/scdna/bigdata/variant_calling/WT_1/cells_readcount'
    ...
    # f1 = pd.read_table('/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/scdna/bigdata/variant_calling/WT_1/WT_1_only_SNPs_candidate.txt',
    #                    header=None, sep=' ')
    f1.sort_values([2, 3], inplace=True)
    f1.columns = ['vac', 'var', 'chr', 'pos', 'ref', 'rc', 'A', 'C', 'G', 'T', 'N', 'vac_rank', 'var_rank', 'rank']
    #
    # f2 = pd.read_table('/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/scdna/bigdata/variant_calling/WT_1/WT_1_bt2_only_SNPs_candidate.txt',
    #                    header=None, sep=' ')
    f2.sort_values([2, 3], inplace=True)
    f2.columns = ['vac', 'var', 'chr', 'pos', 'ref', 'rc', 'A', 'C', 'G', 'T', 'N', 'vac_rank', 'var_rank', 'rank']

    # f3 = pd.read_table('/netscratch/dep_mercier/grp_schneeberger/projects/apricot_leaf/results/scdna/bigdata/variant_calling/WT_1/WT_1_only_SNPs_candidate.sorted.common.regions', header=None)
    ...
    for row in f3.itertuples(index=False):
        ...
        ref2, alt2 = None, None
        try:
            ref1, alt1 = f1_alt[row.chr][row.pos]
        except KeyError:
            ref1, alt1 = None, None
        try:
            ref2, alt2 = f2_alt[row.chr][row.pos]
        except KeyError:
            ref2, alt2 = None, None
        ...
        if ref1 is None:
            ...
            if ref1 != ref2:
                print("Inconsistent reference {}: {}. Please check".format(row.chr, row.pos0) )
                continue
            if alt1 == alt2:
                f3_refs.append(ref1)
                f3_alts.append(alt1)
    ...
    f3_g = [True if f3_alt[i][1] >= M else False for i in f3_loci]
