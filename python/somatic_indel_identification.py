...
def write_pos_only_indels(fin, fout, CAN_N, RC_MIN, RC_MAX, CAN_CNT):
    '''
    Select all positions that have one alternate allele.
    Since, bam-readcount report insertions on the base before and deletion on th exact base, need to compare the adjacent bases (consecutive rows of the bam-readcount output). Only select indel positions that have only one insertion/deletion supported by >= CAN_N reads and no other variation supported by >= CAN_N reads.
    '''
    ...
    with open(fin, 'r') as f:
        ...
        for line in f:
            ...
            if len(ind) > 0:
                ...
                if len(line) > 11:
                    for i in list(range(10, len(line), 2)):
                        if int(line[i]) >= CAN_N: hf += 1
                if hf == 1:
                    df.append(ind)
                ind = list()
            ...
            try:
                ...
                if "+" not in indel:
                    ...
                    ind = [line[0], line[1], line[1], line[2], indel, rc, ar]
...
def getcandidate(args):
    ...
    ## Reading Bed files generated above to perform filtering
    import pybedtools
    beds = deque()
    for f in fouts:
...
def selectgoodcandidate(sample, candidate_readcount, snps_readcount, SAMPLES, CWD):
    ...
    # Get readcount at the indel position in the sample and the total number of alt reads in the other samples
    sample_readcount = deque()
    for pos, v in candidate_readcount[sample].items():
        ...
        for s in SAMPLES:
            if s == sample:
                try:
                    sample_c = int(snps_readcount[s][pos][v[1]])
                except KeyError as e:
    ...
    bad_pos = deque()
    for i in range(len(sample_readcount)):
        # s_rc = sample_readcount[i][0]
        # s_all_rc = sum([sample_readcount[s][i] for s in SAMPLES])
        if sample_readcount[i][1] >= 2*sample_readcount[i][0]:
            bad_pos.append(i)
    bad_pos = set(bad_pos)
    ...
    good_pos = list(set(range(len(sample_readcount))) - bad_pos)
    high_conf = deque()
    for i in good_pos:
    ...
    for pos, v in candidate_readcount[sample].items():
        ...
        for s in SAMPLES:
            if s == sample:
                try:
                    sample_c = int(snps_readcount[s][pos][v[1]])/int(snps_readcount[s][pos]['RC'])
                except KeyError as e:
                    print("ERROR: Candidate does not have any reads: {}".format(pos, s))
